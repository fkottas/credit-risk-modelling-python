...
The repository does not assume that public availability equals permission to republish. Each dataset has a registry record covering publisher, source URL, licence, attribution, redistribution status, access conditions and limitations. Five UCI datasets are downloaded from the publisher with fixed checksums and CC BY 4.0 attribution. The conditional Kaggle examples require the student to download data under their own account after checking current terms; competition files are never bundled. World Bank, FRED and other macro series are reviewed indicator by indicator because underlying rights and revision practices can differ.
