...
1. Create a short-lived branch for one clearly defined change.
...
The book uses eight public UCI datasets whose current catalogue pages state Creative Commons Attribution 4.0 and provide source attribution [R18–R23, R82–R83]. They are historical teaching samples, not a current applicant population. Their use in a worked example does not establish lawful or appropriate use in a lending decision. Several contain demographic or proxy variables that require purpose-specific legal, fairness and privacy review.

| Registry key | Role in the book | Distribution rule | Principal limitation |
|---|---|---|---|
| `uci_south_german` | application scorecard and cost-sensitive decisions | checksum-verified UCI download; source attributed | old, small, oversampled bad cases, no dates |
| `uci_statlog_german` | source-comparison and documentation exercise | checksum-verified UCI download; source attributed | legacy coding problems; not the preferred benchmark |
| `uci_taiwan_credit_card` | behavioural PD and ML challenger | checksum-verified UCI download; source attributed | one geography and reporting period; no out-of-time split |
| `uci_credit_approval` | missing/categorical data laboratory | checksum-verified UCI download; source attributed | target is approval, not default; anonymised meanings |
| `uci_australian_credit_approval` | mixed-type preprocessing | checksum-verified UCI download; source attributed | target is approval; source has replaced missing values |
| `uci_polish_bankruptcy` | corporate low-default experiments | checksum-verified UCI download; source attributed | bankruptcy is not regulatory default; no entity dates |
| `uci_taiwan_bankruptcy` | nonlinear corporate challenger | checksum-verified UCI download; source attributed | severe imbalance and single-jurisdiction context |
| `uci_bank_marketing` | leakage and ordered-source exercises | checksum-verified UCI download; source attributed | target is deposit subscription; contact duration is post-contact |
...
| `world_bank_wdi` | macroeconomic history and satellite-model exercises | official API adapter; WDI record is CC BY 4.0 [R81] | revised country series are not borrower outcomes or forecasts |
| Fannie Mae / Freddie Mac loan-level files | conceptual reference only | no download code and no bundling in this edition | redistribution and commercial-use restrictions require separate permission |
...
defects = inject_teaching_defects_with_manifest(bundle, seed=811)
quality = assess_quality(bundle, defects.frame)

assert len(defects.manifest) > 0
print({
    "injected_defects": len(defects.manifest),
    "failed_rules": quality.failed_rules,
    "critical_failure": quality.critical_failure,
})
```

```output
{'injected_defects': 201, 'failed_rules': ('unique_application_id', 'complete_model_fields', 'range_income', 'domain_product', 'valid_as_of_date', 'flag_post_outcome_columns'), 'critical_failure': True}
```

The 201 manifest rows are field-level changes, not necessarily 201 distinct accounts. The quality report identifies five critical failures and one warning; the manifest allows each observed failure to be traced back to an intentional change.
...
[R39] Kottas, Ferdinantos. *Performance and factor structure of green, grey and red securities in European Union countries*. PhD thesis, National University of Ireland Maynooth, 2025. https://mural.maynoothuniversity.ie/id/eprint/20102/

[R40] Kottas, Ferdinantos. “Empirical Asset Pricing Models for Green, Grey, and Red EU Securities: A Fama–French and Carhart Model Approach.” *Journal of Risk and Financial Management* 18(5), 2025, article 282. https://doi.org/10.3390/jrfm18050282
...
[R77] Siddiqi, Naeem. *Intelligent Credit Scoring: Building and Implementing Better Credit Risk Scorecards*, 2nd ed. Wiley, 2017. https://doi.org/10.1002/9781119282396

[R78] Thomas, Lyn C.; Crook, Jonathan N.; and Edelman, David B. *Credit Scoring and Its Applications*, 2nd ed. Society for Industrial and Applied Mathematics, 2017. https://doi.org/10.1137/1.9781611974560

[R79] Hand, David J., and Henley, William E. “Statistical Classification Methods in Consumer Credit Scoring: A Review.” *Journal of the Royal Statistical Society: Series A* 160(3), 1997, pp. 523–541. https://doi.org/10.1111/j.1467-985X.1997.00078.x

[R80] James, Gareth; Witten, Daniela; Hastie, Trevor; Tibshirani, Robert; and Taylor, Jonathan. *An Introduction to Statistical Learning: with Applications in Python*. Springer, 2023. https://doi.org/10.1007/978-3-031-38747-0

[R81] World Bank. “World Development Indicators.” World Bank Data Catalog; CC BY 4.0 dataset record. https://datacatalog.worldbank.org/search/dataset/0037712/world-development-indicators

[R82] Hofmann, Hans. “Statlog (German Credit Data).” UCI Machine Learning Repository, 1994. https://doi.org/10.24432/C5NC77

[R83] Moro, Sérgio; Rita, Paulo; and Cortez, Paulo. “Bank Marketing.” UCI Machine Learning Repository, 2014. https://doi.org/10.24432/C5K306

[R84] Kottas, Ferdinantos. “Quantitative Credit Risk & AI.” Author-controlled professional profile. Accessed 22 August 2026. https://fkottas.github.io/FerdinantosKottas.github.io/
