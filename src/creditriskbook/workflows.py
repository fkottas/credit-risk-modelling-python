...
def run_end_to_end(
    ...
    *,
    ...
    source_frame = (
        inject_teaching_defects(bundle, seed=seed + 1) if inject_defects else bundle.frame
    )
    ...
    if quality_after.critical_failure:
        raise RuntimeError(
            f"Critical quality failures remain after quarantine: {quality_after.failed_rules}"
        )
    ...
    if dataset_key == "synthetic_retail":
        ...
        result["ecl"] = {
            ...
            "stage_counts": {
                str(int(key)): int(value) for key, value in ecl["stage"].value_counts().items()
            },
